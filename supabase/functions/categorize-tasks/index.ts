this is code now, what edits needed? import "https://deno.land/x/xhr@0.1.0/mod.ts";
  ...
  const resp = await fetch(
    'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-8b:generateContent',
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
  temperature: 0.1,
  maxOutputTokens: 512,                  // you can keep camelCase here if you like
  responseMimeType: "application/json",  // camelCase OK
  responseSchema: {
    type: "array",
    items: {
      type: "object",
      properties: {
        ...
        // IMPORTANT: enum values must be STRINGS for Gemini
        duration: { type: "string", enum: ["15","30","60"] }
      },
      required: ["title","category","duration"]
      // IMPORTANT: remove additionalProperties (Gemini complains)
    }
  }
}
...
const modelDur = toBlock(m?.duration); // instead of validDur.has(m?.duration)...
const duration = modelDur ?? (durationHint ?? guessDuration(title));
